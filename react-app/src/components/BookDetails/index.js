...
const BookDetails = () => {
  ...
  return (
    ...
    <img className = 'book-image-container' src={bookData[0].cover_art} alt='bookcoverimage'></img>
        <div>{dateFix(bookData[0].publication_date)}, {bookData[0].price_paperback ? '$' +bookData[0]?.price_paperback.toFixed(2)+',': null} {bookData[0].price_hardcover ? '$' +bookData[0]?.price_hardcover.toFixed(2)+',': null} {bookData[0].price_eBook ? '$' +bookData[0]?.price_eBook.toFixed(2)+',': null} {bookData[0].title}</div>
    </div>
