...
function CartModal({ currentBookId, isOpen, priceFormat, setIsCartOpen }) {
    ...
    return (
      <>
        ...
        {price === 0.00 && <div>Paperback</div>}
        {price === book.price_paperback && <div>Paperback</div>}
        {price === book.price_hardcover && <div>Hardcover</div>}
        {price === book.price_eBook && <div>eBook</div>}
        <div>${price?.toFixed(2)}</div>
        <div>Qty: 1</div>
